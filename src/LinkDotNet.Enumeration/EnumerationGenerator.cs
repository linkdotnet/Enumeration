...
public sealed class EnumerationGenerator : IIncrementalGenerator
{
    ...
    private static void Emit(SourceProductionContext ctx, EnumerationModel model)
    {
        ...
        {
            sb.AppendLine($"    /// <summary>Gets the <see cref=\"{typeName}\"/> instance for <c>{value}</c>.</summary>");
        ...
        foreach (var value in model.Values)
        {
            sb.AppendLine($"    /// <param name=\"on{value}\">Invoked when the current value is <see cref=\"{value}\"/>.</param>");
        }
